...
var barrier_timer; // Таймер для барьера
...
function startGame() {
    ...
    barrier_timer = setTimeout(createBarrier, 6000);
}
 ...
 * @returns {boolean}
 */
function isBarrierUnit(unit) { //проверка, что змейка не попала сама в себя в новой ячейке
    var check = false;
...
function createBarrier() {
    ...
    barrier_timer = setTimeout(deleteBarrier, 6000);
}

function deleteBarrier() {
    var barrier_cell = document.getElementsByClassName('barrier-unit')[0];
    barrier_cell.setAttribute('class', barrier_cell.getAttribute('class').replace('barrier-unit', ''));
    createBarrier();
}
